...
function makeMediaNode(size, options) {
  // Enforce some styles.
  const enforceStyles = (styles, fullWidth) => {
    const width = getMediaWidth(size, styles, fullWidth);
    const out = styles.filter(style => style.name !== 'width');
    out.push({ name: 'width', value: width });
    return out;
  };

  const opts = Object.assign({}, DEFAULT_OPTIONS.media, options);
  const paddingBottom = `${(size[1] / size[0]) * 100.0}%`;
  const styles = enforceStyles(opts.styles, opts.fullWidth);
