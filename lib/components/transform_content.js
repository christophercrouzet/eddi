...
// Retrieve the metadata for a given deck node element.
function getDeckMeta(node, meta) {
  ...
  const deckMeta = meta.decks[value];
  const medias = (deckMeta.medias === undefined ? [] : deckMeta.medias)
    .map(media => meta.medias[media]);
  return Object.assign({}, deckMeta, { medias });
}


// Retrieve the metadata for a given gallery node element.
function getGalleryMeta(node, meta) {
  const value = joinTextNodeContents(node).trim();
  if (
    !('galleries' in meta)
    || value === undefined
    || !(value in meta.galleries)
  ) {
    return undefined;
  }

  const galleryMeta = meta.galleries[value];
  const medias = (galleryMeta.medias === undefined ? [] : galleryMeta.medias)
    .map(media => meta.medias[media]);
  return Object.assign({}, galleryMeta, { medias });
}
...
// Check if a node represents a deck.
function isDeck(node, meta) {
  return (
    tree.getTagName(node) === 'div'
    && getDeckMeta(node, meta) !== undefined
  );
}


// Check if a node represents a gallery.
function isGallery(node, meta) {
  return (
    tree.getTagName(node) === 'div'
    && getGalleryMeta(node, meta) !== undefined
  );
}
...
// Replace a deck node.
function replaceDeck(node, data, callback) {
  if (data.containerType !== 'deck' || !isDeck(node, data.file.meta)) {
    async.nextTick(callback, null, node, data);
    return;
  }

  const deckMeta = getDeckMeta(node, data.file.meta);
  const deckNode = makeDeckNode(deckMeta);
  tree.insertBefore(tree.getParentNode(node), deckNode, node);
  tree.detachNode(node);
  async.nextTick(callback, null, deckNode, data);
}


// Replace a gallery node.
function replaceGallery(node, data, callback) {
  if (data.containerType !== 'gallery' || !isGallery(node, data.file.meta)) {
    async.nextTick(callback, null, node, data);
    return;
  }

  const galleryMeta = getGalleryMeta(node, data.file.meta);
  const galleryNode = makeGalleryNode(galleryMeta);
  tree.insertBefore(tree.getParentNode(node), galleryNode, node);
  tree.detachNode(node);
  async.nextTick(callback, null, galleryNode, data);
}
...
function step(node, parentData, callback) {
  ...
  if (tree.isElementNode(node)) {
    if (data.contentLevel === 0) {
      ...
      // Any content node.
      fns.push(replaceDeck);
      fns.push(replaceGallery);
