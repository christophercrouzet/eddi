...
const DUMMY_LANGUAGES = ['no-highlight', 'plain', 'text'];
...
const CONTAINERS = {
  carousel: { collectionName: 'carousels', fn: makeCarouselNode },
...
function getLanguage(node) {
  ...
  const filter = (attr) => {
    return DUMMY_LANGUAGES.includes(attr) || attr.startsWith(LANGUAGE_PREFIX);
  };
  ...
  tree.getChildNodes(node)
    ...
    .forEach((textNode) => {
      ...
      if (language === undefined) {
        ...
        language !== null
        && !DUMMY_LANGUAGES.includes(language)
      ) {
...
function markContainer(node, data, callback) {
  ...
  if (data.contentLevel === 1) {
    if (data.containerType === 'gallery' || data.containerType === 'carousel') {
